...
| 字段 | 说明 |
|---|---|
| `code` | 业务错误码，定义见 `com.flydeer.common.exception.ErrorCodes` |
...
| `NeedVerifyException` / `NeedAdminException` / `SmsVerifyException` | **403** Forbidden | `31020` / `31030` / `30070` |
...
| `31030` | `NEED_ADMIN` | 仅对管理员开放 | 403 | `NeedAdminException` | 接口要求 `ADMIN`，当前用户不在 `app.user.admin-ids` |
...
| `42010` | `ATLAS_PUBLISH` | 仅草稿可提交审核 | 400 | `AtlasPublishException` | 非 `DRAFT` 提交审核 |
| `42020` | `ATLAS_NOT_PUBLISHED` | 图集未发布 | 400 | `AtlasNotPublishedException` | 非作者查看未发布图集 |
| `42030` | `ATLAS_APPROVE` | 仅待审可批准发布 | 400 | `AtlasApproveException` | 非 `PENDING` 批准发布 |
| `42110` | `GRAPH_CONTENT_TOO_LARGE` | 图内容超出体积限制 | 400 | `GraphContentTooLargeException` | 单图 / batch-save / list-content 体积超限 |
| `42120` | `GRAPH_PARENT_INVALID` | 目录位置非法 | 400 | `GraphParentInvalidException` | 父图跨图集 / 自引用 / 成环 |
| `42130` | `GRAPH_REV_CONFLICT` | 图已被修改，请刷新后重试 | 409 | `GraphRevConflictException` | `rev` 不匹配；`data.rev` 为服务端当前版本 |
| `42140` | `GRAPH_ID_INVALID` | 图 ID 格式非法或已存在 | 400 | `GraphIdInvalidException` | `graphId` 格式非法、主键冲突或撞已删 id |
...
| `52050` | `GRAPH_NOT_FOUND` | 图不存在或无权操作 | 500 | `GraphNotFoundException` | 图不存在 / 已删除 |
...
1. **先看 `code`，再看 HTTP**：同一业务语义以 `code` 为准；`429` / `401` / `403` 可配合做全局拦截（登录跳转、实名引导、限流提示）。
2. **`31010` / `30020`**：清本地 Access，引导重新登录；Refresh 场景优先调刷新，失败再回登录页。
3. **`31020`**：引导 OAuth 用户走绑手机流程。
4. **`91010` / `91020`**：展示 `message`，并按配置间隔禁用按钮（短信默认约 60s）。
5. **`999999` 与未识别码**：统一「系统繁忙」类文案，勿把服务端堆栈（校验异常时 `message` 可能较技术化）原样给用户。
6. **业务 5xxxx/6xxxx 当前 HTTP 为 500**：不要仅用「HTTP 500 = 系统错误」判断；应用 `code` 区分「用户不存在 / 绑定冲突」等可恢复错误。
...
| 文件 | 职责 |
|---|---|
| `flydeer-common/.../ErrorCodes.java` | 错误码常量 |
| `flydeer-common/.../exception/**` | 异常类与默认文案 |
| `flydeer-controller/.../GlobalExceptionHandler.java` | HTTP 状态与 `ApiResult.fail` 组装 |
| `flydeer-contract/.../ApiResult.java` | 统一响应信封 |
