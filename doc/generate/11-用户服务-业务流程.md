...
|---|---|---|---|
| 手机号短信 | `PHONE` | `SHA-256(手机号 + 盐)` 小写 hex | 是（手机号即实名凭证） |
| Gitee | `GITEE` | 平台用户 ID | 否，需后续绑手机 |
| GitHub | `GITHUB` | 平台用户 ID | 否，需后续绑手机 |

手机号落库规则：

- `phone`：仅存脱敏值（如 `138****5678`），不存明文。
- `phone_hash`：`SHA-256(手机号 + app.user.phone-hash-salt)`，PHONE 注册与 OAuth 绑手机时写入；同渠道下用于唯一性校验。
- 短信发送 / 限流等运行时逻辑仍使用请求中的明文手机号，不落库。

业务含义上的 **verified（已校验）**：

- 手机号渠道注册即视为已校验。
- OAuth 渠道需完成「绑定手机号」后变为已校验，并重新签发带 `verified=true` 的 Token。
- 后续若有接口要求「已校验用户」，未绑手机的 OAuth 用户会被拒绝（改昵称要求已实名；查看资料 / 绑手机仍只要求已登录）。
...
3. 本地默认 `app.sms.mock-enabled: true`：可不接真实短信；联调时注意与生产行为差异。
...
查看资料 / 绑手机需已登录；修改昵称需已登录且已实名。

| 场景 | 流程摘要 |
|---|---|
| 查看自己 | 拉取当前用户资料；手机号脱敏（如 `138****5678`） |
| 修改昵称 | 已实名用户提交新名称（最长 20 字），返回更新后的资料 |
...
| 角色 | 含义 | 关系枚举 |
|---|---|---|
| 代理人 | 发起代理、持有管理权限的一方 | 查询时用 `DELEGATOR`（我代理的） |
| 被代理人 | 被申请管理、需同意的一方 | 查询时用 `DELEGATED`（代理我的） |
...
3. **任一方撤销**  
   - A（代理人）撤销：`relation=DELEGATOR`，`operateId=B`
   - B（被代理人）撤销：`relation=DELEGATED`，`operateId=A`
4. **查询**  
