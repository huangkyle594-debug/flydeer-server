...
todos:
  ...
  - id: account-delegate-api
    content: 仅 nickName 可改 + 用户间授权请求/接受/拒绝/取消/列表
    status: completed
  - id: ratelimit-log
...
| 用户 ID | `user.id`：起始 **10000000**；每次 `max(id) + random(step-min, step-max)`（默认如 1–99），应用层 `IdGenerator` 写入；BIGINT PK，非 AUTO_INCREMENT。`user_delegate.id`：**不对外暴露**，MySQL `AUTO_INCREMENT` 从 **1** 起、固定步长 1 |
...
校验路径：解析 JWT 签名与过期 → 读 `uid` → 查库确认用户存在且 `status` 正常、取最新 `verified` 等 → 注入 `BaseRequest`。

模块落点（沿用现有分层）：

- [`flydeer-struct-mind-contract`](flydeer-struct-mind-contract)：DTO、枚举、`BaseRequest`、API 接口
...
- `id` BIGINT PK（**非 AUTO_INCREMENT**）：首条从 **10000000** 起，之后由 `IdGenerator` 按「当前最大 id + 随机步长」分配；步长上下限在配置中（如 `id.step-min` / `id.step-max`）
...
3. 按 DB 中的 `verified` 等与 `UserLevel` 比对
...
- 注入 `BaseRequest`：`userId`、`channel`、`verified`、`delegatedUserIds`（`List<Long>`：当前用户作为 grantee 且状态为 `ACCEPTED` 的全部授权方 userId；无授权时为空列表）
...
- `PATCH /api/v1/users/me` — 仅可改 `nickName`
...
| 层 | 类 |
|---|---|
| contract | `UserLevel`、`LoginChannel`、`BaseRequest`、Auth/User/Delegate DTO、`AuthApi`/`UserApi`/`DelegateApi` |
| repository | `UserEntity`、`UserDelegateEntity`、`UserMapper`、`UserDelegateMapper` + XML |
| service | `JwtTokenService`、`IdGenerator`（随机步长）、`SmsVerifyService`、`OauthClientService`、`UserService`、`DelegateService`、Properties、`LocalRateLimiter` |
| api | `AuthFacade`、`UserFacade`、`DelegateFacade` |
...
2. 依赖 + Properties + `JwtTokenService`（无状态）+ 单测
